...
class ADB(object):
    ...
    def _connection(self):
        ...
        if 'adbd is already running as root' not in r:
            time.sleep(2)
            self.__adb('connect %s' % self.id.split(":")[0] if ":" in self.id else self.id)  # using tcp/ip CONNECT WAY
            time.sleep(1)
        self.adb('remount')
    ...
    def __adb(self, command):
        ...
        return subprocess.Popen([self.adbd] + command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT).communicate()[
            0]
    ...
    def adb(self, command):
        ...
        return \
            subprocess.Popen(self.adb_command + command, stdout=subprocess.PIPE,
    ...
    def adbshell(self, command):
        ...
        f = os.popen(' '.join(self.adbsh_command + command))
        result = f.read()
        f.close()
        # return \
        #     subprocess.Popen(self.adbsh_command + command, stdout=subprocess.PIPE,
        #                      stderr=subprocess.STDOUT).communicate()[0]
        return result
    ...
    def install(self, apkname):
        ...
        for _retry_times in range(1, 6):
            ...
            print r
            self.retry_connection()
            # self.push(apkname)  # push jar -> phone
        if not pflag:
            raise AdbException(r)


adbtest = ADB('M96GAEP5UKV93')

version = adbtest.adbshell('getprop ^|grep ro.build.inside.id').split(':')[1]
print version
